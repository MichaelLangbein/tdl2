...
We call $P(\mathbf{Y})$ the prior. $\mathbf{\Sigma}$ can be very large, so we assume that it can be modeled by a covariance-function $cov(h)$ which is *only dependent on the distance between two points, not on the points themselves*.
...
Often, statistical tests work *the wrong way round*. You have your hypothesis, like "these groups are different". Then you state the opposite, the *null-hypothesis*. Then you determine how likely your data is under the null-hypothesis - the *p-value*. And than you hope that the p-value is very low.
