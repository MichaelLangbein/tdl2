...
- As a react-dev, one is used to creating hierarchies of elements a bit like this:
    ```
        ...
        node2
            ...
            text
    ```
    - d3 does not work that way. Instead, it first has you create all rects, then all cirlces, then all texts. Think *layers-of-shapes*, not *hierarchies*.
        ```
            rects
                rect1
                rect2
...
function linearGradientComponent(gradientName) {
  ...
  function _getOrAddGrad(defsSelection) {
    ...
    if (!grad.node()) {
      grad = defsSelection.append('linearGradient')
        ...
        .attr('x1', '0%').attr('y1', '0%')
        .attr('x2', '100%').attr('y2', '0%'); // horizontal
    }
    return grad;
  }
  ...
  gradient.stops = (stops) => {_stops = stops; return this;}
