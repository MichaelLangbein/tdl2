...
- Beware of nested `select` statements - each `select` will set the `data` attribute back to the old value
  ...
  ```js
  const activeMessages = activeMessageGroup.selectAll('.message').data([datum], (d) => d.id);
...
const base = select('#svg').style('width', width).style('height', height).append('g');
...
## Important caveats and hacks

1. **Selections**:
   1. `enter`, `exit` selection only works after `select().data()` or `selectAll().data()`, not after `selectAll().datum`
   2. It's useful to be able to count the number of entries in a selection: `mySel.enter().size()`
2. **Line graphs**:
   1. `path`s are hard to animate, but you can do the dasharray-trick: <https://jakearchibald.com/2013/animated-line-drawing-svg/>
   2. One datum for a path is an array of data. You could do `.selectAll('path').datum(myArray)` to add a single line ... but because of 1.1., you're better off with `.selectAll('path').data([myArray])` if you want to handle enter, exit
   3. But now if you pass in an empty array, that still counts as a datum. To make d3 realize that a longer array is a different datum, pass also an id-function: `.selectAll('path').data([myArray], d => d.length)`
...
<html>
  ...
  <body>
    ...
    <p>To explore this behavior, call `displayData` a few times from the web-console.</p>
  </body>
  <script>
...
```js
selectAll('circle').data(someData).enter().append('circle').call(doWithSelection, 'red');
```
...
function createConnection(d) {
  const l = create('svg:line').style('stroke', 'black').style('stroke-width', 1);
  return l.node();
}
...
async function main() {
  function highlightConnections(connectedPaths) {
    const highlightedConnections = baseElement.selectAll('.highlightedConnection').data(connectedPaths);
  ...
  function highlightNodes(nodes) {
    const highlightedNodes = baseElement.selectAll('.highlightedNode').data(nodes);
    highlightedNodes
      .enter()
      .append('circle')
...
function linearGradientComponent(gradientName) {
  ...
  function gradient(selection) {
    ...
    gradient
      ...
      .attr('style', (d) => `stop-color:rgb(${d.color[0]}, ${d.color[1]}, ${d.color[2]}); stop-opacity:${d.opacity}`);
  }
...
testSvg.append('rect').attr('width', 100).attr('height', 30).attr('fill', `url(#myGradient)`);
```
  ...
  function _getOrAddGrad(defsSelection) {
    ...
    if (!grad.node()) {
      ...
      if (_direction === 'horizontal') {
        ...
        grad.attr('x1', '0%').attr('y1', '0%').attr('x2', '0%').attr('y2', '100%');
      }
    }
    return grad;
  }
  ...
  function gradient(selection) {
    ...
    gradient
      ...
      .attr(
        'style',
        (d) =>
          `stop-color:rgb(${color(d.color).r}, ${color(d.color).g}, ${color(d.color).b}); stop-opacity:${d.opacity}`
      );
  }
...
function createLegendDiscrete(
  ...
  const graphic = legendGroup
    ...
    .attr('transform', (d) =>
      direction === 'horizontal' ? `translate(${d.startPos}, 0)` : `translate(0, ${d.startPos})`
    )
    .append('rect')
...
function createLegendContinuous(
  ...
  const placementScale = scaleLinear().domain([minPos, maxPos]).range([0, dominantSize]);

  const percentageScale = scaleLinear().domain([minPos, maxPos]).range([0, 100]);

  const colorScale = scaleColor();
  colorScale.domain(entries.map((e) => e.position)).range(entries.map((e) => e.color));
...
Html and svg famously don't mix too easily.
...
### Adding html inside svg

```ts
// select any svg object
const svgG = svg.select('g');

// add a foreign-object container.
const foreignObject = svgG.append('foreignObject')
  .attr('x', 50)
  .attr('y', 50)
  .attr('width', 250)
  .attr('height', 250);
// Fun fact: foreign-objects can also contain mathml.
// Global css also applies to foreign objects. 
...
const paragraph = htmlContainer.append("p").html("<strong>Hi!</strong>");
```
