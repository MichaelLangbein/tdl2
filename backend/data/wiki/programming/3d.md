...
flowchart LR
    ...
    WorldSpace -- viewMatrix --> CameraObjectSpace
    CameraObjectSpace -- projectionMatrix --> ClipSpace
```
...
**Perspective**:
$$ f = \frac{1}{\tan(\text{field-of-view} / 2)} $$
$$ ri = \frac{1}{\text{near} - \text{far}}$$
$$ ar = \text{aspect-ratio} $$
$$
\begin{bmatrix}
    \text{f / ar} &&  0 && 0 && 0 \\
    0 && f && 0 && 0 \\
    0 && 0 && (\text{near} + \text{far}) ri && -1  \\
    0 && 0 && 2 \text{ near } \text{far } ri && 0 \\
\end{bmatrix}
$$
Followed by removal of the $w$ dimension.
